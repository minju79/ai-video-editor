...
# --- BGM 배경음악 ---
BGM_VOLUME         = 0.12    # BGM 볼륨 (0.0~1.0, 기본 12% — 목소리보다 작게)
BGM_FADE_SEC       = 2.0     # 영상 끝에서 BGM 페이드아웃 시간 (초)
...
def render_final(merged, ass_path, tmpdir, out_path,
    ...
    bgm_volume     : BGM 볼륨 (0.0~1.0, None 이면 BGM_VOLUME 사용)
    """
    vol = bgm_volume if bgm_volume is not None else BGM_VOLUME
    ...
    if bgm_path and os.path.exists(bgm_path):
        shutil.copy(bgm_path, os.path.join(tmpdir, "bgm.mp3"))
        use_bgm = True
    else:
        # assets/bgm.mp3 자동 감지
        auto_bgm = os.path.join(assets_dir, "bgm.mp3")
        if os.path.exists(auto_bgm):
            shutil.copy(auto_bgm, os.path.join(tmpdir, "bgm.mp3"))
            use_bgm = True
    ...
    # ── FFmpeg 입력 목록 ──────────────────────────────────────
    # [0] main.mp4  [1] sound.wav
    # [2..2+N-1] sound1.wav × N   [2+N..] zoom clips
    # [last] bgm.mp3 (있을 때)
    ...
    zoom_base_idx = 2 + N
    bgm_idx = zoom_base_idx + len(zoom_clips)  # BGM은 마지막 입력

    if use_bgm:
        # stream_loop=-1 : BGM 무한 루프 (영상 길이에 맞춰 자동 잘림)
        inputs = ["-stream_loop", "-1", "-i", "bgm.mp3"] + inputs
        # 모든 인덱스를 1씩 올림 (bgm이 [0]이 됨)
        bgm_idx    = 0
        main_idx   = 1
        sound_idx  = 2
        s1_start   = 3
    ...
    # 1. 비디오: ASS 자막 굽기
    fc.append(f"[{main_idx}:v]ass=subs.ass:fontsdir=.[vsub]")
    cur_v = "vsub"
    ...
    # 3. 오디오: 시작 효과음
    fc.append(f"[{sound_idx}:a]aformat=channel_layouts=stereo:sample_rates=48000,adelay=0|0[sa0]")

    # 4. 오디오: 전환 효과음
    for k, t_bound in enumerate(clip_boundaries):
        ms = int(t_bound * 1000)
        fc.append(
            f"[{s1_start+k}:a]aformat=channel_layouts=stereo:sample_rates=48000,"
            f"adelay={ms}|{ms}[sa{k+1}]"
        )

    # 5. 오디오: BGM (있을 때) — 루프 + 볼륨 축소 + 페이드아웃
    n_effects = 1 + N
    if use_bgm:
        video_dur = ffprobe_duration(os.path.join(tmpdir, "main.mp4"))
        fade_st   = max(0.0, video_dur - BGM_FADE_SEC)
        fc.append(
            f"[{bgm_idx}:a]"
            f"aformat=channel_layouts=stereo:sample_rates=48000,"
            f"volume={vol},"
            f"afade=t=out:st={fade_st:.2f}:d={BGM_FADE_SEC}[bgm_out]"
        )
        all_audio = f"[{main_idx}:a]" + "".join(f"[sa{i}]" for i in range(n_effects)) + "[bgm_out]"
        fc.append(f"{all_audio}amix=inputs={2+n_effects}:normalize=0:duration=first[aout]")
    else:
        all_audio = f"[{main_idx}:a]" + "".join(f"[sa{i}]" for i in range(n_effects))
